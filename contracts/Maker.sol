...
contract Maker is ERC20, ReentrancyGuard {
    using SafeMath for uint;
    using SignedSafeMath for int;

    uint public makerAutoId = 1;//order id
    ...
    int256 public makerFundingPayment;
    ...
    uint public openRate = 5000;// 1e4
    uint public removeLiquidityRate = 1000;//1e6
    ...
    bool public addPaused = false;
    bool public removePaused = false;
    ...
    function closeUpdate(
        ...
        int256 tempProfit = makerProfit.add(int(_makerMargin)).add(fundingPayment);
    ...
    function takerDepositMarginUpdate(uint _margin) external nonReentrant _onlyMarket returns (bool){
        require(_margin > 0, 'can not be zero');
    ...
    function addLiquidity(
        ...
        require(amount >= minAddLiquidityAmount, 'Maker: amount < minAddLiquidityAmount');
        (bool isSuccess) = IUser(userAddress).transfer(clearAnchorAddress, sender, amount);
        require(isSuccess, 'Maker: transfer fail');
    ...
    function priceToAddLiquidity(
        ...
        if (balance.add(makerLock) > 0 && totalSupply > 0) {
            ...
            liquidity = order.amount.mul(totalSupply).div(uint(totalUnPNL.add(int(makerLock)).add(int(balance)).add(makerFundingPayment)));
        } else {
        ...
        order.poolTotal = int(balance).add(int(makerLock)).add(totalUnPNL).add(makerFundingPayment);
    ...
    function priceToRemoveLiquidity(
        ...
        require(totalUnPNL <= int(takerTotalMargin) && totalUnPNL * (- 1) <= int(makerLock), 'taker or maker is broken');
        amount = order.liquidity.mul(uint(int(makerLock).add(int(balance)).add(totalUnPNL).add(makerFundingPayment))).div(totalSupply);
    ...
    function makerProfit(uint256 _price) external view returns (int256 unPNL){
        ...
        if (marketType == 1) {
            ...
            unPNL = shortUnPNL.add(longUnPNL);
            if (marketType == 2) {
                unPNL = unPNL.mul(int(IMarket(market).clearAnchorRatio())).div(int(10 ** clearAnchorRatioDecimals));
            }
        }
    }
    ...
    function updateSharePrice(uint price) external view returns (
        ...
        require(totalUnPNL <= int(takerTotalMargin) && totalUnPNL * (- 1) <= int(makerLock), 'taker or maker is broken');
        if (totalSupply > 0) {
            _price = uint(totalUnPNL.add(int(makerLock)).add(int(balance)).add(makerFundingPayment)).mul(10 ** decimals).mul(10 ** sharePriceDecimals).div(totalSupply).div(10 ** clearAnchorDecimals);
        } else {
    ...
    function canRemoveLiquidity(uint _price, uint _liquidity) external view returns (bool){
        if (_price > 0 && totalSupply > 0) {
            ...
            if (totalUnPNL <= int(takerTotalMargin) && totalUnPNL * (- 1) <= int(makerLock)) {
                uint amount = _liquidity.mul(uint(int(makerLock).add(int(balance)).add(totalUnPNL).add(makerFundingPayment))).div(totalSupply);
                if (balance >= amount) {
                    return true;
                }
            }
    ...
    function fundingPaymentUpdate(int256 _fundingPayment) external nonReentrant _onlyMarket {
        if (_fundingPayment != 0) {
            makerFundingPayment = makerFundingPayment.add(_fundingPayment);
        }
    }
